...
function findPossiblePatterns(patterns: Record<string, true>, requestedPatterns: string[]) {
  ...
  for (const request of requestedPatterns) {
    ...
    for(let i = largestAvailablePattern; i > 0; i--) {
      request.split("").forEach((char, charIdx, arr) => {
        const window = arr.slice(charIdx, charIdx + i);
...
const numOfPossiblePatterns = findPossiblePatterns(availablePatternsRecord, requestedPatterns)
